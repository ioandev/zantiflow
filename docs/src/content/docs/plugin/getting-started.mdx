...
| `hostname` | `true` \| `false` | allow the real-hostname lookup (default off; needs the RunCommands permission, only requested when on and `machine_name` is `real`) |
| `control_long_poll` | `true` \| `false` | hold the control poll for ≈1 s reactions (default on; `false` = fixed ~5 s poll) |
| `debug` | `true` \| `false` | verbose local troubleshooting lines in Zellij's own log (default off; attention/send/claude-activity transitions — never your token or pane content) |
